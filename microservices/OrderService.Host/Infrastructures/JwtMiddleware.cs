...
//            string uid, permissions, userName;

//            var roleList = jwtToken.Claims.Where(x => x.Type.Equals(ClaimTypes.Role)).ToList();
//            uid = jwtToken.Claims.SingleOrDefault(x => x.Type.Equals(UserIdKeyName)).Value;
//            permissions = jwtToken.Claims.SingleOrDefault(x => x.Type.Equals(PermissionsKeyName)).Value;
...
//                //new Claim(PermissionsKeyName , permissions),
//            };
