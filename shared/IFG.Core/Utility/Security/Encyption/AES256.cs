...
{
    private static AES256 _encryptioOperator;

    private static void Init()
        => _encryptioOperator = (_encryptioOperator ?? new AES256());

    public static string Aes256Encrypt(this string text, string passphrase)
    {
        Init();
        return _encryptioOperator.Encrypt(text, passphrase);
    }

    public static string Aes256Decrypt(this string encrypted, string passphrase)
    {
        Init();
        return _encryptioOperator.Decrypt(encrypted, passphrase);
    }
}